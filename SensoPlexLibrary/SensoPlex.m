...
- (void) stopScanningForBLEPeripherals {
    ...
    self.bleConnectionMode = SensoPlexBLEConnectionModeAutoConnection;
...
- (void)discoverBLEPeripherals
{
    ...
    self.bleConnectionMode = SensoPlexBLEConnectionModeDiscoveryOnly;
}
...
{
    [self.centralManager cancelPeripheralConnection:_blePeripheral];
}
...
-(BOOL) getConfig {
    ...
    @try {
        ...
        // commands down
        if ( !self.writeCharacteristic ) {
            LogError(@"No Write Characteristic available to get config.");
            return NO;
        }
        
        // write our "Get Config" command to the write characteristic
        CBCharacteristicWriteType writeType = CBCharacteristicWriteWithResponse;
        CBCharacteristic *characteristic = self.writeCharacteristic;
        CBUUID *uuid = characteristic.UUID;
        const char *charDesc = [self CBUUIDToString:uuid];
        
        Byte bytes[2] = {(Byte)BLE_CMD_GETCONFIG, 0};
        NSData *cmdData = [NSData dataWithBytes:&bytes length:2];
        if ( self.logBLEStats )
            Log(@"** Writing %@ to %s", cmdData, charDesc);
        [self.blePeripheral writeValue:cmdData forCharacteristic:characteristic type:writeType];
...
-(BOOL) setLED:(SensoPlexLEDState)ledState {
    @try {
        ...
        Byte lightColor = 0x0;
        if(ledState == LEDSystemControl)
        {
            lightColor = (Byte)BLE_CMD_LEDSYSCONTROL;
        }
        else
        {
...
-(BOOL) getDataStreamingConfig {
    @try {
        if ( !self.writeCharacteristic ) {
            LogError(@"No Characteristic available to get streaming config.");
            return NO;
        }
        
        Log(@"* Retrieving data streaming config...");
        Byte bytes[2] = {(Byte)BLE_CMD_STREAMGETCONFIG, 0};
        NSData *cmdData = [NSData dataWithBytes:&bytes length:2];
        [self.blePeripheral writeValue:cmdData forCharacteristic:self.writeCharacteristic type:CBCharacteristicWriteWithResponse];
...
//            self.packetDataLock = [[NSLock alloc] init];
        ...
        // send our command down to start capturing data
        Byte bytes[2] = {(Byte)BLE_CMD_LOGENABLE, 0x01};
        NSData *cmdData = [NSData dataWithBytes:&bytes length:2];
        [self.blePeripheral writeValue:cmdData forCharacteristic:self.writeCharacteristic type:CBCharacteristicWriteWithResponse];
    ...
    @catch (NSException *exception) {
        LogError(@"Error trying to start logging data.  %@", exception.description);
        return NO;
    }
}
...
-(BOOL) stopLoggingData {
    @try {
        ...
        Log(@"* Stopping Data Loggging...");
        Byte bytes[2] = {(Byte)BLE_CMD_LOGENABLE, 0x0};
        NSData *cmdData = [NSData dataWithBytes:&bytes length:2];
        [self.blePeripheral writeValue:cmdData forCharacteristic:self.writeCharacteristic type:CBCharacteristicWriteWithResponse];
    ...
    @catch (NSException *exception) {
        LogError(@"Error trying to stop logging data.  %@", exception.description);
        return NO;
    }
}

// Set the clock on the sensor
// AHN Mar 2015
//---------------
- (BOOL) setRTC
//---------------
{
    @try {
        if ( !self.writeCharacteristic ) {
            LogError(@"No Characteristic available to set RTC.");
            return NO;
        }
...
//        };
        ...
        Byte bytes[8] = {(Byte)BLE_CMD_SETRTC
            ,(Byte) info->tm_sec
            ,(Byte) info->tm_min
            ,(Byte) info->tm_hour
            ,(Byte) info->tm_mday
            ,1 // dummy
            ,(Byte) info->tm_mon+1
            ,(Byte) (info->tm_year + 1900 - 2000)
        };
        NSData *cmdData = [NSData dataWithBytes:&bytes length:8];
        [self.blePeripheral writeValue:cmdData forCharacteristic:self.writeCharacteristic type:CBCharacteristicWriteWithResponse];
        return YES;
    }
    @catch (NSException *exception) {
        LogError(@"Error trying to set RTC.  %@", exception.description);
        return NO;
    }
} // setRTC
...
-(BOOL) getLogStatus {
    @try {
        ...
        Log(@"* Retrieving log status...");
        Byte bytes[2] = {(Byte)BLE_CMD_LOGGETSTATUS, 0};
        NSData *cmdData = [NSData dataWithBytes:&bytes length:2];
        [self.blePeripheral writeValue:cmdData forCharacteristic:self.writeCharacteristic type:CBCharacteristicWriteWithResponse];
    ...
    @catch (NSException *exception) {
        LogError(@"Error trying to get log status.  %@", exception.description);
        return NO;
    }
}
...
-(BOOL) getLoggingConfig {
    @try {
        if ( !self.writeCharacteristic ) {
            LogError(@"No Characteristic available to get logging config.");
            return NO;
        }
        
        Log(@"* Retrieving log config...");
        Byte bytes[2] = {(Byte)BLE_CMD_LOGGETCONFIG, 0};
        NSData *cmdData = [NSData dataWithBytes:&bytes length:2];
        [self.blePeripheral writeValue:cmdData forCharacteristic:self.writeCharacteristic type:CBCharacteristicWriteWithResponse];
    ...
    @catch (NSException *exception) {
        LogError(@"Error trying to get logging config.  %@", exception.description);
        return NO;
    }
}
...
- (BOOL) getLogFirstRecord
{
    @try {
        if ( !self.writeCharacteristic ) {
            LogError(@"No Characteristic available log first record.");
            return NO;
        }
        
        Log(@"* Retrieving first record...");
        Byte bytes[2] = {(Byte)BLE_CMD_LOGFIRSTGETRECORD, 0};
        NSData *cmdData = [NSData dataWithBytes:&bytes length:2];
        [self.blePeripheral writeValue:cmdData forCharacteristic:self.writeCharacteristic type:CBCharacteristicWriteWithResponse];
    ...
    @catch (NSException *exception) {
        LogError(@"Error trying to get first log record.  %@", exception.description);
        return NO;
    }
}
...
- (BOOL) getLogRecordWithNo:(Byte)num
{
    @try {
        ...
        Log(@"* Retrieving record with num %x...", num);
        Byte bytes[2] = {(Byte)BLE_CMD_LOGGETRECORD, num};
        NSData *cmdData = [NSData dataWithBytes:&bytes length:2];
        [self.blePeripheral writeValue:cmdData forCharacteristic:self.writeCharacteristic type:CBCharacteristicWriteWithResponse];
    ...
    @catch (NSException *exception) {
        LogError(@"Error trying to log record.  %@", exception.description);
        return NO;
    }
}
...
- (void) onConfigDataParsed:(SensorDataConfig *)data
{
    // todo:
}
...
{
    if ( self.delegate && [self.delegate respondsToSelector:@selector(onSensorLogStatusParsed:)] ) {
        [self.delegate onSensorLogStatusParsed:data];
    }
}

- (void) onSensorLogConfigDataParsed:(NSArray *)data
{
    if([self.delegate respondsToSelector:@selector(onSensorLogConfigDataParsed:)])
    {
        [self.delegate onSensorLogConfigDataParsed:data];
    }
}

- (void) onStreamDataConfigParsed:(SensorDataLogStreamConfig *)data
{
    if([self.delegate respondsToSelector:@selector(onSensorStreamConfigDataParsed:)])
    {
        [self.delegate onSensorStreamConfigDataParsed:data];
    }
}
...
- (void) onSensorLogRecordParsed:(SensorLogData*)data
{
    ...
    {
        [self.delegate onSensorLogRecordParsed:data];
    }
}

- (void) onSensorLogRecordComplete
{
    if([self.delegate respondsToSelector:@selector(onSensorLogRecordComplete)])
    {
...
- (BOOL) shouldConnectToPeripheral:(CBPeripheral*)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI {
    ...
    BOOL connect = NO;
    if ( [self peripheralMatchesName:name] ) {
        connect = YES;
    } else if ( [self peripheralMatchesName:advertisedName] ) {
        connect = YES;
    }
    else {
...
- (BOOL)peripheralMatchesName:(NSString*)name
{
//    [name isEqualToString:@"SP-10BN"]
...
- (void) onSensoPlexReady {
    ...
    // let's get the stream config data as well
    [self getDataStreamingConfig];
    
    // let's get the log config data as well
    [self getLoggingConfig];
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI
{
    ...
    switch (self.bleConnectionMode) {
        case SensoPlexBLEConnectionModeAutoConnection:
            ...
            break;
        case SensoPlexBLEConnectionModeDiscoveryOnly:
