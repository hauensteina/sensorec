...
-(void) processPDIPacket:(Byte*)bytes length:(int)length {
    @try {
        ...
        switch (cmd ) {
            ...
            case PDI_CMD_CONFIG: {
                Log(@"Received Config.");
                [self processConfigData:bytes length:length];
                break;
            }
            case PDI_CMD_LOGGETCONFIG: {
                Log(@"Received LOG GET Config.");
                [self processDataLogConfigPacket:bytes length:length];
                break;
            }
            case PDI_CMD_LOGCONFIG: {
                Log(@"Received Log Config/Log Record.");
                [self processDataLogRecord:bytes length:length];
                break;
            }
            ...
            case PDI_CMD_LOGSTATUS: {
                ...
                [self processLogStatusPacket:bytes lenght:length];
                break;
            }
            case PDI_CMD_STATUS: {
            ...
            case PDI_CMD_STREAMGETCONFIG: {
                // call our method to parse the streaming config packet
                Log(@"Received STREAM GET Config.");
...
- (void) processFirmwareVersionPacket:(Byte*)bytes length:(int)length {
    ...
    if ( length > structSize ) {
        ...
        if ( [self.delegate respondsToSelector:@selector(onFirmwareVersionParsed:)] ) {
...
- (void) processStatusPacket:(Byte*)bytes length:(int)length {
    ...
    if ( length > structSize ) {
        ...
        if ( [self.delegate respondsToSelector:@selector(onSensorStatusParsed:chargerState:batteryVolts:)] ) {
            [self.delegate onSensorStatusParsed:status.Model chargerState:status.ChargerState batteryVolts:volts];
        }
    }
}

// process / parse a sensor config data
- (void) processConfigData:(Byte*)bytes length:(int)length {
    // copy the bytes into our structure
    ...
    if ( length > structSize ) {
        ...
        SensorDataConfig *data = [SensorDataConfig new];
        data.boardAddress = @"";
        // set address
        int max = 6;
        for(int i=0; i<max; ++i)
        {
            Byte comp = (Byte)cfg.bd_addr[i];
            data.boardAddress = [data.boardAddress stringByAppendingFormat:@"%x", comp];
            if(i < max-1)
            {
                data.boardAddress = [data.boardAddress stringByAppendingString:@":"];
            }
        }
        DLog(@"%@", data.boardAddress);
        // set other data
        
        
        if ( [self.delegate respondsToSelector:@selector(onConfigDataParsed:)] ) {
            [self.delegate onConfigDataParsed:data];
        }
    }
}
...
//    // magnetometer - for tests only ================
//    //60 80 00 E1 00 00 00 CE FF FF FF A7 00 00 00 CD DF
//    Byte arr[] = {0x60, 0x80, 0x00, 0xe1, 0x00, 0x00, 0x00, 0xce, 0xff, 0xff, 0xff, 0xa7, 0x00, 0x00, 0x00, 0xcd};
//    bytes = &arr[0];
//    length = 16;
//    _use12BytesForAccelsAndGyros = YES;
//    // ====================================
    ...
    if ( options & LOGDATA_GYROS ) {
        ...
        if ( _use12BytesForAccelsAndGyros ) {
            // let's get the gyro data (4 bytes each : Q16 values) - 12 bytes
    ...
    if ( options & LOGDATA_ACCELS ) {
        ...
        // but since 0.1.9, we use 4
        if ( _use12BytesForAccelsAndGyros ) {
            // let's get the accel data (4 bytes each : Q16 values) - 12 bytes
    ...
    if ( options & LOGDATA_QUATERNION ) {
        // extract our 4 float quaternion values (Q30 values) - 16 bytes
    ...
    if ( options & LOGDATA_COMPASS ) {
        ...
        if ( _use12BytesForAccelsAndGyros )
        {
            // extract our 3 float compass values (Q16 values) - 12 bytes
            sensorData.magnetometerX = [self q16FloatFromBytes:&data.data[curByteIndex]];
            curByteIndex += 4;
            
            sensorData.magnetometerY = [self q16FloatFromBytes:&data.data[curByteIndex]];
            curByteIndex += 4;
            
            sensorData.magnetometerZ = [self q16FloatFromBytes:&data.data[curByteIndex]];
            curByteIndex += 4;
        }
        else
        {
    ...
    if ( options & LOGDATA_RSSI ) {
        // 1 Bytes of BLE RSSI data
//        Byte lowByte = data.data[curByteIndex++];
//        Byte highByte = data.data[curByteIndex++];
//        SInt16 rssi = [self wordFromBytes:lowByte highByte:highByte];
//        sensorData.bleRSSI = rssi;
        
        Byte byte = data.data[curByteIndex++];
        sensorData.bleRSSI = byte;
    }
    ...
    if ( [self.delegate respondsToSelector:@selector(onSensorDataParsed:)] ) {
        [self.delegate onSensorDataParsed:sensorData];
    }
}

// process / parse log record data
- (void) processDataLogRecord:(Byte*)bytes length:(int)length {
    // make sure we check our firmware version
    if ( !_checkedFirmwareVersionForStreamOptions ) {
        [self checkFirmwareVersionForStreamOptions];
    }
    ...
    // if length is 0 then we parsed all the records
    if(data.Length == 0)
    {
        [self.delegate onSensorLogRecordComplete];
        return;
    }
    
    SensorLogData *sensorData = [[SensorLogData alloc] init];
    
    sensorData.typeId = (int)data.Id;
    sensorData.recNumber = [self wordFromBytes:data.RecNum[0] highByte:data.RecNum[1]];
    sensorData.sensorLogMask = [self wordFromBytes:data.Sensors[0] highByte:data.Sensors[1]];
    
    UInt16 options = sensorData.sensorLogMask;
...
//    // iterate through each bit of data included based on the options
    ...
    // let our delegate know that we have sensor data
    if ( [self.delegate respondsToSelector:@selector(onSensorLogRecordParsed:)] ) {
        [self.delegate onSensorLogRecordParsed:sensorData];
    }
}

// process / parse a stream data config todo
- (void) processDataStreamConfigPacket:(Byte*)bytes length:(int)length {
    LOGTYPE_STRUCT config;
    int structSize = sizeof(config);
    if ( length > structSize ) {
        memcpy(&config, bytes+1, sizeof(config));
        
        SensorDataLogStreamConfig *data = [SensorDataLogStreamConfig new];
        data.enabled = (BOOL)config.Enabled;
        data.enabledSensorsMask = [self wordFromBytes:config.Sensors[0] highByte:config.Sensors[1]];
        data.interval = [self wordFromBytes:config.Interval[0] highByte:config.Interval[1]];
        
        if ( self.delegate ) {
            [self.delegate onStreamDataConfigParsed:data];
        }
    }
}

// process / parse a data logging config todo
- (void) processDataLogConfigPacket:(Byte*)bytes length:(int)length {
    
    NSData *dat = [NSData dataWithBytes:bytes length:length];
    DLog(@"%@", dat);
    
    LOGCONFIG_STRUCT config;
    int structSize = sizeof(config);
    if ( length > structSize + 2 )
    {
        bytes += 3;
        NSMutableArray *types = [NSMutableArray arrayWithCapacity:3];
        for(int i = 0; i<3; ++i)
        {
            LOGTYPE_STRUCT logType;
            memcpy(&logType, bytes, sizeof(logType));
            SensorDataLogStreamConfig *data = [[SensorDataLogStreamConfig alloc] init];
            data.enabled = logType.Enabled;
            data.enabledSensorsMask = [self wordFromBytes:logType.Sensors[0] highByte:logType.Sensors[1]];
            data.interval = [self wordFromBytes:logType.Interval[0] highByte:logType.Interval[1]];
            [types addObject:data];
            bytes += sizeof(logType);
        }

        if ( self.delegate ) {
            [self.delegate onSensorLogConfigDataParsed:types];
        }
    }
}

// process / parse a log status data
- (void) processLogStatusPacket:(Byte*)bytes lenght:(int)length
{
    ...
    if ( length > structSize ) {
        ...
        SensorDataLogStatus *logStatus = [SensorDataLogStatus new];
    
        logStatus.logNumberOfRecords = [self wordFromBytes:status.Log_NumOfRecords[0]
                                                  highByte:status.Log_NumOfRecords[1]];

        logStatus.logTotalBytes = [self dwordFromBytes:status.Log_TotalBytes[0]
                                                   lowByte2:status.Log_TotalBytes[1]
                                                   highByte:status.Log_TotalBytes[2]
                                                  highByte2:status.Log_TotalBytes[3]];
        
        logStatus.logUsedBytes = [self dwordFromBytes:status.Log_UsedBytes[0]
                                              lowByte2:status.Log_UsedBytes[1]
                                              highByte:status.Log_UsedBytes[2]
                                             highByte2:status.Log_UsedBytes[3]];
        
        
        logStatus.enabled = (BOOL)status.Enabled;
        
        [logStatus logClass];
        
        if ( [self.delegate respondsToSelector:@selector(onSensorLogStatusParsed:)] ) {
            [self.delegate onSensorLogStatusParsed:logStatus];
        }
    }
}
